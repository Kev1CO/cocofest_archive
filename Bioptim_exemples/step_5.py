...
from custom_package.ding_model import (
    ...
    CustomDynamicsIntensityFrequency
)
...
def prepare_ocp(
    ...
    ding_models = [DingModelIntensityFrequency() for i in range(n_stim)]  # Gives DingModel as model for n phases
    ...
    for i in range(n_stim):
        dynamics.add(CustomDynamicsIntensityFrequency.declare_ding_variables,
                     dynamic_function=CustomDynamicsIntensityFrequency.custom_dynamics, phase=i)
    ...
    stim_intensity_bounds = Bounds(np.array([pulse_intensity_min]*n_stim), np.array([pulse_intensity_max]*n_stim),
                                  interpolation=InterpolationType.CONSTANT)
    initial_intensity_guess = InitialGuess(np.array([0]*n_stim))
    parameters.add(parameter_name="pulse_intensity",
...
def main():
    ...
    pulse_intensity_min = 0  # minimum pulse duration during the phase (stimulation)
    pulse_intensity_max = 150  # maximum pulse duration during the phase (stimulation)
    ...
    ocp = prepare_ocp(n_stim=n, time_min=time_min, time_max=time_max,
                      pulse_intensity_min=pulse_intensity_min, pulse_intensity_max=pulse_intensity_max,
                      fourier_coeff=fourier_coeff)
