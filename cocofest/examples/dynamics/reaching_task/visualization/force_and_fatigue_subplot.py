...
for j in range(2):
    for i in range(3):
        ...
        axs[i][j].plot(data_minimize_fatigue["time"], data_minimize_fatigue["states"][force_muscle_keys[index]],
                       lw=5)
        axs[i][j].text(0.5, 0.9, f"{muscle_names[index]}", transform=axs[i][j].transAxes, ha="center", va="center", fontsize=14, weight="bold", font="Times New Roman")
...
fig.text(0.025, 0.5, 'Force (N)', ha='center', va='center', rotation='vertical', fontsize=18, weight="bold", font="Times New Roman")
fig.legend(['Force', 'Fatigue'], loc='upper right', ncol=1, prop={'family': 'Times New Roman', 'size': 14, 'weight': 'bold'})
plt.show()
...
for i in range(2):
    ...
    axs[i].text(0.05, 0.9, f"{joint_keys[i]}", transform=axs[i].transAxes, ha="left", va="center",
                   fontsize=14, weight="bold", font="Times New Roman")
    labels = axs[i].get_xticklabels() + axs[i].get_yticklabels()
    [label.set_fontname("Times New Roman") for label in labels]
    [label.set_fontsize(14) for label in labels]

fig.text(0.05, 0.5, 'Joint angle (°)', ha='center', va='center', rotation='vertical', fontsize=18, weight="bold", font="Times New Roman")
...
[label.set_fontname("Times New Roman") for label in labels]
[label.set_fontsize(14) for label in labels]
fig.text(0.05, 0.5, 'Fatigue percentage (%)', ha='center', va='center', rotation='vertical', fontsize=18, weight="bold", font="Times New Roman")
axs.text(0.75, 96.3, 'Time (s)', ha='center', va='center', fontsize=18, weight="bold", font="Times New Roman")
plt.legend(['Force', 'Fatigue'], loc='upper right', ncol=1, prop={'family': 'Times New Roman', 'size': 14, 'weight': 'bold'})
plt.show()
