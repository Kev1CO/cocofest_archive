...
frequency_path = [r"/result_file/pulse_apparition_minimize_muscle_force.pkl",
                  r"/result_file/pulse_apparition_minimize_muscle_fatigue.pkl"]

duration_path = [r"/result_file/pulse_duration_minimize_muscle_force.pkl",
                 r"/result_file/pulse_duration_minimize_muscle_fatigue.pkl"]

intensity_path = [r"/result_file/pulse_intensity_minimize_muscle_force.pkl",
                  r"/result_file/pulse_intensity_minimize_muscle_fatigue.pkl"]

chosen_graph_to_plot_path = (frequency_path if chosen_graph_to_plot == "frequency" else
                             duration_path if chosen_graph_to_plot == "duration" else
                             intensity_path if chosen_graph_to_plot == "duration" else
                             None)
...
for i in range(3):
    for j in range(2):
        ...
        axs[i][j].text(.025, .975, f'{muscle_names[index]}', transform=axs[i][j].transAxes, ha="left", va="top", weight='bold', font="Times New Roman")
        ...
        if i == 0 and j == 0:
            axs[i][j].plot(data_minimize_force["time"], data_minimize_force["states"][force_muscle_keys[index]][0], ms=4, linewidth=5.0,
                           label="Minimizing force")
            axs[i][j].plot(data_minimize_fatigue["time"], data_minimize_fatigue["states"][force_muscle_keys[index]][0], ms=4, linewidth=5.0,
